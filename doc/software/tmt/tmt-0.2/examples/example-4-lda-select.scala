...
import edu.stanford.nlp.tmt.stage.LDAMetadata;      // new in 0.2
...
val nonEmptyRows = text.data.items.map(_.number).toList;
val numTrain = nonEmptyRows(nonEmptyRows.size * 4 / 5);
  
// build a training dataset
val training = LDADataset(text ~> Take(numTrain));
 
// build a test dataset, using term index from the training dataset 
val testing  = LDADataset(text ~> Drop(numTrain), training.meta[LDAMetadata.TermIndex]);
...
for (numTopics <- List(4,5,6,7,8,9,10,15,20)) {
  ...
  val model = TrainGibbsLDA(output, training, modelParams, GibbsLDA.DefaultTrainingParams);
