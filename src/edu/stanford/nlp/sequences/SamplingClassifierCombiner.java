 ...
 * @author Chris Cox   
 */
public class SamplingClassifierCombiner{
...
//   private static final String DEFAULT_AUX_CLASSIFIER_PATH="/u/nlp/data/ner/dist/muc.7class.crf.gz";
   ...
   */
  public SamplingClassifierCombiner(Properties p) 
  ...
  private void moveAnswerFieldsToNERField(List<CoreLabel> sent, List<CoreLabel> ner) {
    if (sent.size() != ner.size()) {
      throw new RuntimeException("Size Mismatch!");
    }
    Iterator<CoreLabel> nerIter = ner.iterator();
    for (CoreLabel m : sent) {
      m.setNER(nerIter.next().get(AnswerAnnotation.class));
      m.set(AnswerAnnotation.class, null);
    }
  }
