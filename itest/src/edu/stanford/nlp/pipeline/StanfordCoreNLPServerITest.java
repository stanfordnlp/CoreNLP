...
public class StanfordCoreNLPServerITest {
  ...
  public void testSemgrexAnnotation() throws IOException {
    String expected = "sentence { pattern { match { matchIndex: 3 node { name: \"verb\" matchIndex: 3 } node { name: \"obj\" matchIndex: 5 } sentenceIndex: 0 semgrexIndex: 0 } semgrexIndex: 0 } sentenceIndex: 0 }".replaceAll(" ", "");
    String query = "The dog ate a fish";
    byte[] message = query.getBytes("utf-8");
    Properties props = new Properties();
    props.setProperty("annotators", "tokenize,ssplit,pos,parse");
