...
public class PhraseTable implements Serializable
{
  ...
  public void readPhrases(String filename, boolean checkTag) throws IOException
  {
    ...
    while ((line = br.readLine()) != null) {
      if (checkTag) {
        String[] columns = tabPattern.split(line, 2);
        if (columns.length == 1) {
          addPhrase(columns[0]);
        } else {
...
/*        if (!phraseAdded && i == wordList.size() - 1) {
          // add to list
          Phrase newphrase = new Phrase(wordList, phraseText, tag);
        ...
        if (!phraseAdded && nMaps == 0) {
          ...
          if (lookupList.size() > MAX_LIST_SIZE) {
            ...
            /*    if (oldphrase.wordList.size() <= i+1) {
                  //newList.add(oldphrase);
                  newMap.put(PHRASE_END, oldphrase);
                } else {
                  newMap.put(oldphrase.wordList.getWord(i+1), oldphrase);
                } */
              } else  {
                throw new RuntimeException("Unexpected class in list " + obj.getClass() + " while converting list to map");
              }
            }
            //tree.put(word, newList);
