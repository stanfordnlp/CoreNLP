 ...
 */
  ...
  public static void updateEntities(HashMap entities, String entityTokensRaw,String entityLabel,
      boolean ignoreCase){
    ...
    if  (entities.containsKey(entityTokens)){
      int[] counts = (int[])entities.get(entityTokens);
  ...
  public HashMap namedEntitiesLabelCounts(BufferedReader br, boolean corpusLevel,
...
/*
  public void pruneEntities(HashMap entities){
  ...
  public static void printMajority (HashMap entities, String entityTokensRaw,String entityLabel,
      boolean ignoreCase){
...
//        if (! (  entitySoFar.trim().equals("") || entitySoFar.trim().equals("~") ))
 ...
 //       else
  ...
  public void DocumentLevelMajority(String file,boolean ignoreCase,
    ...
    Vector hashMapsofDocuments = new Vector();
    BufferedReader br = new BufferedReader(new FileReader(file));
    while (true) {
      HashMap entities = namedEntitiesLabelCounts(br, false, 1, 3, ignoreCase, entityLevel);
      if (entities == null) {
        break;
      } else {
...
//        if (! (  entitySoFar.trim().equals("") || entitySoFar.trim().equals("~") ))
        if (lineNo > 1) {
          printMajority((HashMap) hashMapsofDocuments.get(docNo), entitySoFar,
    ...
    printMajority((HashMap) hashMapsofDocuments.get(hashMapsofDocuments.size() - 1),
  ...
  public void printMajoritySuperString (HashMap entities, String entityTokens,String entityLabel){
  ...
  /*
    ...
    for (int i=0;i < allEntityTokens.length;i++){
      if (isSubEntity(allEntityTokens[i],entityTokens)){
            int[] counts = (int [])entities.get(allEntityTokens[i]);
  ...
  public static HashMap subStringToSuperStringIndex(HashMap entities,int superStringPosition){
...
// Same as printMajoritySuperString except that it takes an extra argument
  ...
  public static void printMajoritySuperStringIndexed(HashMap entities, HashMap subToSuperIndex,
  ...
  int counts[] = (int[])entities.get(entityTokens);
    ...
    // Need to get the final counts here...
    int[] totalSuperstringCounts = new int[5];
    Vector superEntities = (Vector) subToSuperIndex.get(entityTokens);
    if (superEntities != null) {


      for (int i = 0; i < superEntities.size(); i++) {
        int[] counts = (int[]) entities.get((String) superEntities.get(i));
  ...
  public void CorpusLevelSuperstringMajority(String file,int superStringPosition,
    ...
    boolean entityLevel = true;
     ...
     HashMap entities = namedEntitiesLabelCounts(br,true,1,3,ignoreCase,entityLevel);
    br.close();
    HashMap subToSuperIndex = subStringToSuperStringIndex(entities,superStringPosition);
...
//        if (! (  entitySoFar.trim().equals("") || entitySoFar.trim().equals("~") ))
 ...
 //       if (! (  entitySoFar.trim().equals("") || entitySoFar.trim().equals("~") ))
  ...
  public void DocumentLevelSuperstringMajority(String file,int superStringPosition,
   ...
   // stores all hashmaps, one for each doc
    Vector hashMapsofDocuments = new Vector();
    Vector hashMapsofSubtoSuperStrings = new Vector();
    BufferedReader br = new BufferedReader(new FileReader(file));
   while (true){
    HashMap entities = namedEntitiesLabelCounts(br,false,1,3,ignoreCase,entityLevel);
    if (entities == null)
...
//        if (! (  entitySoFar.trim().equals("") || entitySoFar.trim().equals("~") ))
        if (lineNo > 1){
         // printMajoritySuperString((HashMap)hashMapsofDocuments.get(docNo),entitySoFar,prevEntityLabel);
          printMajoritySuperStringIndexed((HashMap)hashMapsofDocuments.get(docNo),
              (HashMap)hashMapsofSubtoSuperStrings.get(docNo),entitySoFar,prevEntityLabel,ignoreCase);
 ...
 //   printMajoritySuperString((HashMap)hashMapsofDocuments.get(hashMapsofDocuments.size() - 1),
   //     entitySoFar,prevEntityLabel);
    printMajoritySuperStringIndexed((HashMap)hashMapsofDocuments.get(hashMapsofDocuments.size() - 1),
        (HashMap)hashMapsofSubtoSuperStrings.get(hashMapsofDocuments.size() - 1),
        entitySoFar,prevEntityLabel,ignoreCase);
  ...
  public void printMajoritySubString (HashMap entities, String entityTokens,String entityLabel){
...
//        if (! (  entitySoFar.trim().equals("") || entitySoFar.trim().equals("~") ))
        if (lineNo > 1)
          printMajoritySubString((HashMap)hashMapsofDocuments.get(docNo),entitySoFar,prevEntityLabel);
 ...
 //       else
  //        System.out.println("");
    ...
    printMajoritySubString((HashMap)hashMapsofDocuments.get(hashMapsofDocuments.size() - 1),
        entitySoFar,prevEntityLabel);
