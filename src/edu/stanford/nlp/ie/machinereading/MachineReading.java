...
public class MachineReading {
   ...
   */
  public static final Logger logger = Logger.getLogger(MachineReading.class.getName());
  
  /*
   * general options
   */
  @Argument("which GenericDataSetReader to use (needs to match the corpus in question)")
  @Argument.Policy(ArgumentPolicy.REQUIRED)
  private Class<GenericDataSetReader> datasetReaderClass;
  
  @Argument("which GenericDataSetReader to use for aux data set (needs to match the corpus in question)")
  @Argument.Policy(ArgumentPolicy.OPTIONAL)
  private Class<GenericDataSetReader> datasetAuxReaderClass;
  
  @Argument("If false, use the original head (and worse) finding mechanism in GenericDataSetReader.  This option is primarily around for legacy purposes.")
  @Argument.Default("true")
  private boolean useNewHeadFinder;
  
  @Argument("verbosity of the corpus reader")
  @Argument.Default("SEVERE")
  private String readerLogLevel;
  
  @Argument("if false, we do not attempt to serialize the train/test corpora after reading")
  @Argument.Default("true")
  private boolean serializeCorpora;
  
  @Argument("if true, regenerate span annotations for trees")
  @Argument.Default("true")
  private boolean forceGenerationOfIndexSpans = true;

  /*
   * entity extraction options
   */
  @Argument("where to store/load the serialized entity extraction model")
  @Argument.Default("")
  protected String serializedEntityExtractorPath;
  
  @Argument("where to store the serialized sentences containing the results of entity extraction")
  protected String serializedEntityExtractionResults;
  
  // TODO this option is temporary and should be removed when (if?) gazetteers get
  // folded into feature factories
  @Argument("location of entity gazetteer file (if you're using one) -- this is a temporary option")
  private String entityGazetteerPath;
  
  @Argument("entity extractor class to use")
  @Argument.Default("edu.stanford.nlp.ie.machinereading.BasicEntityExtractor")
  private Class<BasicEntityExtractor> entityClassifier;
  
  @Argument("comma-separated list of ResultsPrinter subclasses to use for printing the results of entity extraction")
  @Argument.Default("")
  private String entityResultsPrinters;
  
  /*
   * relation extraction options
   */
  @Argument("where to store/load the serialized relation extraction model")
  @Argument.Default("")
  protected String serializedRelationExtractorPath;
  
  @Argument("where to store the serialized sentences containing the results of relation extraction")
  protected String serializedRelationExtractionResults;
  
  @Argument("FeatureFactory class to use for generating features from relations for relation extraction")
  @Argument.Default("edu.stanford.nlp.ie.machinereading.RelationFeatureFactory")
  private Class<RelationFeatureFactory> relationFeatureFactoryClass;
  
  @Argument("relation mention factory class to use")
  @Argument.Default("edu.stanford.nlp.ie.machinereading.structure.RelationMentionFactory")
  private Class<RelationMentionFactory> relationMentionFactoryClass;
  
  @Argument("comma-separated list of feature types to generate for relation extraction")
//  @Argument.Default("all")
  private String relationFeatures;
  
  @Argument("comma-separated list of ResultsPrinter subclasses to use for printing the results of relation extraction")
  @Argument.Default("edu.stanford.nlp.ie.machinereading.RelationExtractorResultsPrinter")
  private String relationResultsPrinters;
  
  @Argument("if true, the relation extraction model trains using predicted rather than gold entity mentions")
  @Argument.Default("false")
  private boolean trainRelationsUsingPredictedEntities;
  
  @Argument("if true, the relation extraction model is evaluated using predicted rather than gold entity mentions")
  @Argument.Default("false")
  private boolean testRelationsUsingPredictedEntities;
  
  @Argument("If true, it creates automatically negative examples by generating all combinations between EntityMentions in a sentence")
  @Argument.Default("true")
  private boolean createUnrelatedRelations;
  
  @Argument("If true, it does not create any lexicalized features from the first argument (needed for KBP)")
  @Argument.Default("false")
  private boolean doNotLexicalizeFirstArg;
  
  // XXX temporary NFL deadline based hack
  @Argument("If true, the relation extractor will use ExtractorMerger for annotation (not training)")
  @Argument.Default("false")
  private boolean useRelationExtractionModelMerging;
  
  @Argument("comma-separated list relation types to skip during training")
  @Argument.Default("")
  private String relationsToSkipDuringTraining;
  
  @Argument("additional (probably domain-dependent) annotator to postprocess relations")
  private Class<Extractor> relationExtractionPostProcessorClass;
  
  /*
   * event extraction options
   */
  @Argument("where to store/load the serialized event extraction model")
  @Argument.Default("")
  protected String serializedEventExtractorPath;
  
  @Argument("where to store the serialized sentences containing the results of event extraction")
  protected String serializedEventExtractionResults;
  
  @Argument("comma-separated list of ResultsPrinter subclasses to use for printing the results of event extraction")
  @Argument.Default("")
  private String eventResultsPrinters;
  
  @Argument("if true, the event extraction model trains using predicted rather than gold entity mentions")
  @Argument.Default("false")
  private boolean trainEventsUsingPredictedEntities;
  
  @SuppressWarnings("unused")
  @Argument("if true, the event extraction model is evaluated using predicted rather than gold entity mentions")
  @Argument.Default("false")
  private boolean testEventsUsingPredictedEntities;
  
  /*
   * global, domain-dependent options
   */
  @Argument("consistency checker class to use")
  private Class<Extractor> consistencyCheck;
  
  /*
   * training options
   */
  @Argument("path to the training file/directory")
  protected String trainPath;
  
  @Argument("path to the aux training file/directory")
  protected String auxDataPath;
  
  @Argument("where to store the serialized training sentences objects")
  @Argument.Policy(ArgumentPolicy.REQUIRED)
  protected String serializedTrainingSentencesPath;
  
  @Argument("where to store the serialized aux training sentences objects")
  @Argument.Policy(ArgumentPolicy.OPTIONAL)
  protected String serializedAuxTrainingSentencesPath;
    
  @Argument("if true, load a serialized model rather than training a new one")
  @Argument.Default("false")
   ...
   */
  @Argument("if true, don't run evaluation (implies forceRetraining)")
  @Argument.Default("false")
  protected boolean trainOnly;

  @Argument("path to the testing file/directory")
  protected String testPath;

  @Argument("where to store the serialized test sentence objects")
  protected String serializedTestSentencesPath;
 
  @Argument("whether to extract entities, or use gold-standard entities for relation/event extraction")
  @Argument.Default("true")
  protected boolean extractEntities;
  
	@Argument("whether we should extract relations")
  @Argument.Default("true")
  protected boolean extractRelations;
  
  @Argument("whether we should extract events")
  @Argument.Default("true")
  protected boolean extractEvents;
  
  /*
   * cross-validation options
   */
  @Argument("if true, run cross-validation")
  @Argument.Default("false")
  protected boolean crossValidate;
  
  @Argument("number of partitions in training data for cross validation")
  @Argument.Default("5")
  private int kfold;
  ...
  @Argument("Pct of train partition to use for training (RELMS experiment)")
  @Argument.Default("1.0")
  private double percentageOfTrain;
  ...
  public static MachineReading makeMachineReadingForAnnotation(
    ...
    if (mr.entityExtractor != null)
      mr.entityExtractor.setLoggerLevel(level);
  ...
  public static MachineReading makeMachineReading(String [] args) throws IOException {
    ...
    System.err.println("PERCENTAGE OF TRAIN: " + mr.percentageOfTrain);
    
    // convert args to properties (TODO: we really should be switching to Properties completely...)
    ...
    Level readerLogLevel = Level.parse(mr.readerLogLevel.toUpperCase());
  ...
  public List<String> run() throws Exception {
    ...
    Annotation aux = null;
    if ((extractEntities && !haveSerializedEntityExtractor) || 
        (extractRelations && !haveSerializedRelationExtractor) || 
        (extractEvents && !haveSerializedEventExtractor) ||
        forceRetraining || crossValidate){
      ...
      training = loadOrMakeSerializedSentences(trainPath, reader, new File(serializedTrainingSentencesPath));
			if (auxReader != null) {
			  logger.severe("Reading auxiliary dataset from " + auxDataPath + "...");
				aux = loadOrMakeSerializedSentences(auxDataPath, auxReader, new File(
    ...
    if(! trainOnly){
      ...
      //
      if (extractEntities && serializedEntityExtractionResults != null) 
        IOUtils.writeObjectToFile(mergedPredictions[ENTITY_LEVEL], serializedEntityExtractionResults);
      if (extractRelations && serializedRelationExtractionResults != null) 
        IOUtils.writeObjectToFile(mergedPredictions[RELATION_LEVEL],serializedRelationExtractionResults);
      if (extractEvents && serializedEventExtractionResults != null)
        IOUtils.writeObjectToFile(mergedPredictions[EVENT_LEVEL],serializedEventExtractionResults);
  ...
  protected List<String> printTask(String taskName, Set<ResultsPrinter> printers, Annotation gold, Annotation pred) {
    ...
    for (ResultsPrinter rp : printers){
      ...
      retMsg.add(msg);
      logger.severe("Overall " + taskName + " results, using printer " + rp.getClass() + ":\n" + msg);
    }
    return retMsg;
  }
  ...
  protected void train(Annotation training, int partition) throws Exception {
    ...
    if (extractRelations) {
      ...
      if (partition != -1)
        logger.info("In partition #" + partition);
    ...
    for (CoreMap sent : dataset.get(CoreAnnotations.SentencesAnnotation.class)) {
      ...
      for (RelationMention rm : relationMentions) {
        ...
        if (rm.replaceGoldArgsWithPredicted(entityMentions)) {
  				logger.info("Successfully mapped all arguments in relation mention: " + rm);
  				newRels.add(rm);
  			} else {
  				logger.info("Dropped relation mention due to failed argument mapping: " + rm);
  			}
  		}
  ...
  protected Annotation annotate(Annotation testing, int partition) {
    ...
    if (extractEntities) {
      ...
      for (ResultsPrinter rp : entityResultsPrinterSet){
        ...
        logger.info("Entity extraction results " + (partition != -1 ? "for partition #" + partition : "") + " using printer " + rp.getClass() + ":\n" + msg);
      }
    ...
    if (extractRelations) {
      ...
      Annotation predicted = (testRelationsUsingPredictedEntities ? predictions[ENTITY_LEVEL][partitionIndex] : AnnotationUtils.deepMentionCopy(testing));
      ...
      if (relationExtractionPostProcessor == null) {
        relationExtractionPostProcessor = makeExtractor(relationExtractionPostProcessorClass);
      }
      if (relationExtractionPostProcessor != null) {
        logger.info("Using relation extraction post processor: " + relationExtractionPostProcessorClass);
        relationExtractionPostProcessor.annotate(predicted);
      }
      
      for (ResultsPrinter rp : getRelationResultsPrinterSet()){
        String msg = rp.printResults(testing, predicted);
        logger.info("Relation extraction results " + (partition != -1 ? "for partition #" + partition : "") + " using printer " + rp.getClass() + ":\n" + msg);
      }
      ...
      if (consistencyChecker == null) {
        consistencyChecker = makeExtractor(consistencyCheck);
      }
      if (consistencyChecker != null) {
        logger.info("Using consistency checker: " + consistencyCheck);
        consistencyChecker.annotate(predicted);
        
        for (ResultsPrinter rp : entityResultsPrinterSet){
          String msg = rp.printResults(testing, predicted);
          logger.info("Entity extraction results AFTER consistency checks " + (partition != -1 ? "for partition #" + partition : "") + " using printer " + rp.getClass() + ":\n" + msg);
        }
        for (ResultsPrinter rp : getRelationResultsPrinterSet()){
          String msg = rp.printResults(testing, predicted);
          logger.info("Relation extraction results AFTER consistency checks " + (partition != -1 ? "for partition #" + partition : "") + " using printer " + rp.getClass() + ":\n" + msg);
        }
      }
  ...
  private void assignSyntacticHeadToEntities(Annotation corpus) {
    ...
    for(CoreMap sent: corpus.get(SentencesAnnotation.class)){
      ...
      if (forceGenerationOfIndexSpans) {
        tree.indexSpans(0);  
      }
      assert(tree != null);
  ...
  protected void makeDataSets(Annotation training, Annotation testing, Annotation auxDataset) {
    if(! crossValidate){
      ...
      assert(kfold > 1);
      datasets = new Pair[kfold];
      AnnotationUtils.shuffleSentences(training);
      for (int partition = 0; partition < kfold; partition++) {
        int begin = AnnotationUtils.sentenceCount(training) * partition / kfold;
        int end = AnnotationUtils.sentenceCount(training) * (partition + 1) / kfold;
        logger.info("Creating partition #" + partition + " using offsets [" + begin + ", " + end + ") out of " + AnnotationUtils.sentenceCount(training));
      ...
      predictions = new Annotation[3][kfold];
    }
  }
  ...
  private void makeResultsPrinters(String[] args) {
    entityResultsPrinterSet = makeResultsPrinters(entityResultsPrinters, args);
    setRelationResultsPrinterSet(makeResultsPrinters(relationResultsPrinters, args));
    eventResultsPrinterSet = makeResultsPrinters(eventResultsPrinters, args);
  }
  
  private Set<ResultsPrinter> makeResultsPrinters(String classes, String [] args) {
    logger.info("Making result printers from " + classes);
  ...
  private GenericDataSetReader makeReader(Properties props) {
    try {
      if(reader == null){
        try {
          reader = datasetReaderClass.getConstructor(Properties.class).newInstance(props);
        } catch(java.lang.NoSuchMethodException e) {
          // if no c'tor with props found let's use the default one
          reader = datasetReaderClass.getConstructor().newInstance();
        }
      }
    } catch (Exception e) {
      throw new RuntimeException(e);
    ...
    reader.setUseNewHeadFinder(useNewHeadFinder);
    return reader;
  }
  ...
  private GenericDataSetReader makeAuxReader() {
    try {
			if (auxReader == null) {
				if (datasetAuxReaderClass != null) {
					auxReader = datasetAuxReaderClass.getConstructor().newInstance();
				}
			}
		} catch (Exception e) {
      throw new RuntimeException(e);
  ...
  protected Annotation loadOrMakeSerializedSentences(
    ...
    if (serializeCorpora && serializedSentences.exists() && !forceParseSentences) {
      ...
      if(serializeCorpora)
        logger.info("These sentences will be serialized to " + serializedSentences.getAbsolutePath());
