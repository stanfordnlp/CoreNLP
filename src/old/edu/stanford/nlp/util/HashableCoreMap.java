...
public class HashableCoreMap extends ArrayCoreMap {
  ...
  private final Set<Class<? extends Key<CoreMap, ?>>> immutableKeys;
  ...
  public HashableCoreMap(Map<Class<? extends Key<CoreMap, ?>>,Object> hashkey) {
    int keyHashcode = 0;
    int valueHashcode = 0;
    
    for (Map.Entry<Class<? extends Key<CoreMap, ?>>,Object> entry : hashkey.entrySet()) {
      ...
      super.set((Class<? extends Key>)entry.getKey(),
          entry.getValue());
    }
   ...
   */
  @SuppressWarnings("unchecked")
  public HashableCoreMap(ArrayCoreMap other, Set<Class<? extends Key<CoreMap, ?>>> hashkey) {
    super(other);
    
    int keyHashcode = 0;
    int valueHashcode = 0;
    
    for (Class<? extends Key<CoreMap, ?>> key : hashkey) {
      // NB it is important to compose these hashcodes in an order-independent
      // way, so we just add them all here.
  ...
  public boolean equals(Object o) {
    if (o instanceof HashableCoreMap) {
      ...
      for (Class<? extends Key<CoreMap, ?>> key : immutableKeys) {
