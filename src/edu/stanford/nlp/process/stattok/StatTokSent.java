...
public class StatTokSent{
	...
	*/
	private ArrayList<Pair<CoreLabel, String>> splitToken(Pair<CoreLabel, String> tokenAndClass, Map<String, String[]> multiWordRules){
		...
		/*Get token parts from dictionary. Convert String[] into List<String> */ /* TODO: add List<String> to multiWordRules Map to avoid conversion */
		 ...
		 * Each token is created from the part. Its lenght is used to assign begin and end position, starting from the end position of the original token. 
		 * The last considered token's (the first) begin and end position
		 * are set based on the remaining characters.
		 * Then, the token is added in first position of the return list, with the corresponding class (C if is not the last token, the original class of the main token if it is the last)
		*/
