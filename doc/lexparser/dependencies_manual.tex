...
quite effective in relation extraction applications.
...
The current representation contains approximately 53 grammatical
...
\noindent\textbf{\emph{abbrev}: abbreviation modifier}\\
...
\>  ``The Australian Broadcasting Corporation (ABC)" \> \> \emph{abbrev}(Corporation, ABC)\\
\end{tabbing}
...
\begin{tabbing}
\hspace{1cm} \= xxxxxxxxxxxxxxxxxxxxxxxxxx\= \hspace{.5cm}\=  \kill
\>  ``Sam, my brother" \> \> \emph{appos}(Sam, brother)\\
\>  ``Bill (John's cousin)" \> \> \emph{appos}(Bill, cousin)\\
\end{tabbing}
...
\begin{tabbing}
\hspace{1cm} \= xxxxxxxxxxxxxxxxxxxxxxxxxx\= \hspace{.5cm}\=  \kill
\> ``Reagan has died"  \> \>  \emph{aux}(died, has)\\
 \> ``He should leave'' \> \> \emph{aux}(leave, should)\\
\end{tabbing}
...
\noindent\textbf{\emph{complm}: complementizer}\\
A complementizer of a clausal complement (\emph{ccomp}) is the word introducing it. It will be the subordinating conjunction ``that'' or ``whether''.
\begin{tabbing}
\hspace{1cm} \= xxxxxxxxxxxxxxxxxxxxxxxxxx\= \hspace{.5cm}\=  \kill
\>  ``He says that you like to swim" \> \> \emph{complm}(like, that)\\
\end{tabbing}
...
A marker of an adverbial clausal complement (\emph{advcl}) is the word introducing it. It will be a subordinating conjunction different from ``that'' or ``whether'': e.g.\@ ``because'', ``when'', ``although'', etc.
\begin{tabbing}
\hspace{1cm} \=XXXXXXXXXXXXXXXXXXXXXXXXXXXX\= \hspace{2cm}\=  \kill
 \> ``Forces engaged in fighting after insurgents attacked" \> \> \emph{mark}(attacked, after)\\
\end{tabbing}
...
\noindent\textbf{\emph{purpcl}: purpose clause modifier}\\
A purpose clause modifier of a VP is a clause headed by ``(in order) to" specifying a purpose. At present the system only recognizes ones that have ``in order to'' as otherwise the system is unable to distinguish from the surface representations between these and open clausal complements (\emph{xcomp}). It can also recognize fronted ``to" purpose clauses in sentences.
\begin{tabbing}
\hspace{1cm} \=XXXXXXXXXXXXXXXXXXXXXXXXXXXX\= \hspace{1cm}\=  \kill
\> ``He talked to him in order to secure the account" \>  \> \emph{purpcl}(talked, secure)\\
\end{tabbing}
...
\end{tabbing}
This analysis is used only for relative words which are not the subject of the relative clause.  Relative words which act as the subject of a relative clause are analyzed as a \emph{nsubj}.
...
\begin{tabbing}
\hspace{0.3cm} \= \texttt{nonCollapsedDependenciesSeparated} \= \hspace{.3cm} \= \kill
\> \texttt{basicDependencies} \>\> basic dependencies\\
\> \texttt{collapsedDependencies} \>\> collapsed dependencies (not necessarily a tree structure)\\
\> \texttt{CCPropagatedDependencies} \>\>  collapsed dependencies with propagation of conjunct \\
\>                                  \> \> dependencies (not necessarily a tree structure) \\
\>                             \> \> [this representation is the default, if no option is specified]\\
\> \texttt{treeDependencies} \>\> collapsed dependencies that preserve a tree structure\\
\> \texttt{nonCollapsedDependencies} \>\> non-collapsed dependencies: basic dependencies as well as\\
\>									\>\> the extra ones which do not preserve a tree structure\\
\> \texttt{nonCollapsedDependenciesSeparated} \>\> non-collapsed dependencies where the basic dependencies\\
...
\hspace*{-0.375in}%
Java example: \\
\small\texttt{LexicalizedParser lp = \\
\hspace*{0.2in}LexicalizedParser.loadModel("edu/stanford/nlp/models/lexparser/englishPCFG.ser.gz",\\
...
A number of dependency parsers have now been trained to parse directly to the basic Stanford Dependencies, including MaltParser and MSTParser.  If desired, these parses can then be postprocessed to the collapsed or CCprocessed representation using the \texttt{-conllxFile} option of \texttt{EnglishGrammaticalStructure}, as discussed above.  Or our conversion tool can convert the output of other constituency parsers to the Stanford Dependencies representation.  For more information on other parser options, see:
\begin{quote}
