...
For these examples we will work with [chapter 13 of _Ulysses_ by James Joyce](James-Joyce-Ulysses-ch13.txt). You can download it if you want to follow along.
...
## CoreNLP doesn’t need to take much time or space
...
It can do this with the sample text while giving Java just 20MB of memory:
...
and the annotation speed is about 6500 tokens per second – another 5 times faster.
...
The only thing you can do about 1. is to either remove annotators that you do not need or to make choices for smaller annotators.  These models are what fills the large models jar. They are even larger when they are uncompressed and represented in memory. Here are some examples.

Currently, the most memory-requiring models in the default pipeline are the neural network or statistical coreference. If you run without them, you can annotate the sample document in 2GB of RAM:
...
But once you include coreference – here, we’ve explicitly listed the annotators, but these are the default options – then the system really needs 4GB of RAM:
...
Until v.3.6.0, the default parser was `englishPCFG.ser.gz`. It was small and quick to load, but takes quadratic space and cubic time on sentence length. Bad news! If you have long sentences, you should either limit the maximum length parsed with a flag like `-parse.maxlen 70` or choose a different parser.
...
If you are working using dependency parses, the fastest choice is to use the fast, dependency parse features only statistical coreference model. If you’ve already paid for constituency parsing, the fastest choice for coreference is deterministic coreference (dcoref), but it’s the least accurate.  Neural-english coref is a reasonable choice for higher quality coreference. Several of the coreference models have some properties that will speed up their application to long documents:

- `dcoref` has `dcoref.maxdist` with value the maximum number of tokens back to look for a coreferent mention.
- Both statistical and neural `coref` have `coref.maxMentionDistance` and `coref.maxMentionDistanceWithStringMatch` which allow two variants of the same kind of limiting how far back you look for coreferent mentions. 
