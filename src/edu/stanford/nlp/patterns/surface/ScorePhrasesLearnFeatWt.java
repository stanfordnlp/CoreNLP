...
public class ScorePhrasesLearnFeatWt extends PhraseScorer {
  ...
  public edu.stanford.nlp.classify.Classifier learnClassifier(Map<String, List<CoreLabel>> sents, String label, boolean forLearningPatterns,
      TwoDimensionalCounter<String, SurfacePattern> wordsPatExtracted, Counter<SurfacePattern> allSelectedPatterns, Counter<String> dictOddsWordWeights) throws IOException {
...
//    }
  ...
  public Counter<String> scorePhrases(Map<String, List<CoreLabel>> sents, String label, TwoDimensionalCounter<String, SurfacePattern> terms,
    ...
    Counter<String> scores = new ClassicCounter<String>();
    edu.stanford.nlp.classify.Classifier classifier = learnClassifier(sents, label, forLearningPatterns, wordsPatExtracted, allSelectedPatterns, dictOddsWordWeights);
    for (Entry<String, ClassicCounter<SurfacePattern>> en : terms.entrySet()) {
      double score = this.scoreUsingClassifer(classifier, en.getKey(), label, forLearningPatterns, en.getValue(), allSelectedPatterns, dictOddsWordWeights);
  ...
  public RVFDataset<String, ScorePhraseMeasures> choosedatums(String label, boolean forLearningPattern, Map<String, List<CoreLabel>> sents, Class answerClass, String answerLabel,
      ...
      Counter<SurfacePattern> allSelectedPatterns, Counter<String> dictOddsWordWeights) {
      ...
      for (int i = 0; i < sent.length; i++) {
        ...
        if (chooseThis) {
          ...
          if (forLearningPattern) {
            feat = getPhraseFeaturesForPattern(label, l.word(), dictOddsWordWeights);
          } else {
            feat = getFeatures(label, l.word(), wordsPatExtracted.getCounter(l.word()), allSelectedPatterns, dictOddsWordWeights);
          }
  ...
  Counter<ScorePhraseMeasures> getPhraseFeaturesForPattern(String label, String word, Counter<String> dictOddsWordWeights) {
    ...
    if (constVars.usePatternEvalSemanticOdds) {
      double dscore = this.getDictOddsScore(word, dictOddsWordWeights);
  ...
  public double scoreUsingClassifer(edu.stanford.nlp.classify.Classifier classifier, String word, String label, boolean forLearningPatterns,
      Counter<SurfacePattern> patternsThatExtractedPat, Counter<SurfacePattern> allSelectedPatterns, Counter<String> dictOddsWordWeights) {
    ...
    if (scoreClassifierType.equals(ClassifierType.DT)) {
      ...
      if (forLearningPatterns)
        feat = getPhraseFeaturesForPattern(label, word, dictOddsWordWeights);
      else
        feat = this.getFeatures(label, word, patternsThatExtractedPat, allSelectedPatterns, dictOddsWordWeights);
